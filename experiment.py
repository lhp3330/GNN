...
from utils.load_data import split_dataset, load_dataset, get_optimizer, get_metric_type, get_folds, \
    generate_single_random_split, split_dataset2, generate_split_mask_data
from utils.set_seeds import set_seed
from arguments.gumbel_args import GumbelArgs
from arguments.env_args import EnvArgs
from arguments.act_args import ActArgs
from model.CoGNN import CoGNN
...
class Experiment(object):
    def __init__(self, args):
        ...
        logging.basicConfig(
            ...
            filename=f'logs/1/{self.dataset_name}.log',
            filemode='w')
    ...
    def run(self):
        ...
        for fold in folds:
            ...
            # data = split_dataset(dataset, fold) # 1 2 7
            # data = generate_single_random_split(dataset, self.out_dim)
            # data = generate_split_mask_data(dataset, self.out_dim)
            # data = dataset
    ...
    def single_fold(self, fold, data, env_args, act_args, gumbel_args):
        ...
        # # model.eval()
        # with torch.no_grad():
        #     _, _, z_init = model(data.x, data.edge_index, M_ppnp=self.M_ppnp)
        #
        # self.kmeans = KMeans(n_clusters=self.out_dim, n_init=20)
        # self.kmeans.fit(z_init.detach().cpu().numpy())
        # model.cluster_centers.data = torch.tensor(self.kmeans.cluster_centers_).to(device=self.device)
    ...
    def train(self, data, model, optimizer, epoch):
        ...
        #     model.eval()
        #     with torch.no_grad():
        #         _, _, z_init = model(data.x, data.edge_index)
        #
        #     kmeans = KMeans(n_clusters=self.out_dim, n_init=20)
        #     kmeans.fit(z_init.detach().cpu().numpy())
        #     model.cluster_centers.data = torch.tensor(kmeans.cluster_centers_).to(device=self.device)
        #     model.train()
        #
        # # # 如果过了 Warmup 阶段，加上聚类 Loss
        # if epoch >= self.warmup_epochs:
        #     loss_cluster = self.get_clustering_loss(embedding, model.cluster_centers)
        #     loss = loss + self.cluster_weight * loss_cluster
