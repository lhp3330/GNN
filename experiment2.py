...
from utils.load_data import split_dataset, load_dataset, get_optimizer, get_metric_type, get_folds, \
    generate_single_random_split, split_dataset, generate_split_mask_data
from utils.set_seeds import set_seed
from arguments.gumbel_args import GumbelArgs
from arguments.env_args import EnvArgs
from arguments.act_args import ActArgs
from model.CoGNN2 import CoGNN
from utils.load_data import get_optimizer
from utils.load_data import get_metric_type
...
class Experiment2(object):
    def __init__(self, args):
        ...
        logging.basicConfig(
            ...
            filename=f'logs/2/{self.dataset_name}.log',
            filemode='w')
    ...
    def run(self):
        ...
        print(f'\nFinal metric: {np.mean(metric_list):.4f} \u00B1 {np.var(metric_list) * 100:.2f}')
        p = np.var(metric_list)
    ...
    def single_fold(self, fold, data, env_args, act_args, gumbel_args):
        ...
        with tqdm.tqdm(total=self.max_epochs, file=sys.stdout) as pbar:
            test_metric_list, sota = self.train_and_test(data=data, model=model, optimizer=optimizer, pbar=pbar, fold=fold)
            # test_metric = np.array([m.cpu().item() for m in test_metric_list])
        return sota
    ...
    def train(self, data, model, optimizer):
        model.train()
        optimizer.zero_grad()
        logits, _ = model.forward(x=data.x, edge_index=data.edge_index)
        train_loss = self.criterion(logits[data.train_mask], data.y[data.train_mask])
        train_loss.backward()
        optimizer.step()
        del train_loss, logits

    def test(self, data, model, mask):
        model.eval()
        with torch.no_grad():
            logtis, _ = model(x=data.x, edge_index=data.edge_index)
            node_mask = getattr(data, f'{mask}_mask')
            loss = self.criterion(logtis[node_mask], data.y[node_mask])
            metric = self.metric(logtis[node_mask], data.y[node_mask])

        return loss, metric

    def draw_plot(self, data_list):
        pass
